...
from git_mirror.utils import safe_env
...
def test_load_env_prefers_home_env_when_token_present():
    ...
    with (
        patch("git_mirror.utils.safe_env.LOADED", False),
        patch("git_mirror.utils.safe_env.Path.home", return_value=Path("/fake/home")),
        patch("git_mirror.utils.safe_env.load_dotenv") as mock_load_dotenv,
...
def test_load_env_reports_errors_and_keeps_going():
    with (
        patch("git_mirror.utils.safe_env.LOADED", False),
        patch("git_mirror.utils.safe_env.Path.home", side_effect=RuntimeError("boom")),
        patch("git_mirror.utils.safe_env.console.print") as mock_print,
    ):
        safe_env.load_env()
