...
def github_repo_manager(tmp_path):
    return GithubRepoManager("token", tmp_path, "user_login", prompt_for_changes=False)


@pytest.mark.parametrize(
    "pypi_owner_name,expected_owner", [(None, True), ("correct_owner", True), ("wrong_owner", False)]
)
def test_check_pypi_publish_status(github_repo_manager, tmp_path, pypi_owner_name, expected_owner):
