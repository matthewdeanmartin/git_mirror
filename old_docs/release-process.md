...
1. The `Create Draft Release` workflow keeps a draft GitHub Release synchronized from the changelog.
1. A maintainer reviews and publishes that draft release.
1. The `Release` workflow bumps version files, opens a release PR, builds the package, and publishes to PyPI through trusted publishing.
1. After PyPI publishing succeeds, merge the generated release PR.
